...
class PPEMasterList(Document):
    pass


def notify_ppe_masterlist_responsible_persons(doc, method):
    ...
    send_ppe_masterlist_mail(doc, recipients)


def send_ppe_masterlist_mail(doc, recipients):
    document_link = frappe.utils.get_url_to_form("PPE Masterlist", doc.name)

    subject = f"PPE Masterlist Assigned: {doc.ppe_name or doc.name}"

    current_user = frappe.session.user
    created_by = frappe.db.get_value("User", current_user, "full_name") or current_user

    fields_to_show = [
        ("PPE Name", doc.ppe_name),
        ("Life of PPE", doc.life_of_ppe),
        ("Inspection Frequency", doc.inspection_frequency),
        ("Remarks", doc.reamarks),
    ...
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 620px; margin: auto; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden;">
        <div style="background-color: #1a3c6e; padding: 20px; text-align: center;">
            <h2 style="color: #ffffff; margin: 0;">🦺 PPE Masterlist Assignment</h2>
        </div>
        <div style="padding: 25px; background-color: #ffffff;">
            <p style="font-size: 15px; color: #333;">Hi,</p>
            <p style="font-size: 15px; color: #333;">
                You have been assigned as a <b>Responsible Person</b> for the following PPE Masterlist record by <b>{created_by}</b>. Please review the details below.
            </p>
