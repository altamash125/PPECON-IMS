...
#  IMS portal dashboard backend  (v4.1)
...
def get_training_stats(year=None):
    ...
    for i in range(3, -1, -1):
        q_start = getdate(add_months(cur_q_start, -3 * i))
        q_end = add_days(getdate(add_months(q_start, 3)), -1)
...
def _electricity_date_field():
    for f in ("date", "end_date", "reading_date"):
        if _has_field("Electricity Consumption", f):
            return f
    return "date"
...
def get_electricity_summary():
    ...
    date_f = _electricity_date_field()

    try:
        row = frappe.db.sql(
            "SELECT MAX(YEAR(`{d}`)) FROM `tabElectricity Consumption` WHERE docstatus < 2"
            .format(d=date_f)
        )
    ...
    def month_totals(year):
        try:
            rows = frappe.db.sql(
                """SELECT MONTH(`{d}`) AS m, SUM(`{f}`) AS total
                   FROM `tabElectricity Consumption`
                   WHERE docstatus < 2 AND YEAR(`{d}`) = %s
                   GROUP BY MONTH(`{d}`)""".format(d=date_f, f=field),
                (year,), as_dict=True,
            )
    ...
    prior_year_total = sum(prior.values())
    prior_active_months = len([v for v in prior.values() if v > 0])
    prior_avg = (prior_year_total / prior_active_months) if prior_active_months else 0
    ...
    for i, mname in enumerate(month_names, start=1):
        ...
        baseline = prior_actual if prior_actual else prior_avg
        target = round(baseline * 0.95, 2) if baseline else 0
...
# def get_training_stats(year=None):
#     """
#     Shows training coverage across the last 4 quarters that actually
#     have Training Event data — not bound to a single calendar year,
#     so training spanning Dec 2025 + Aug 2026 both show up correctly.
#     """
#     total_employees = _safe_count("Employee", {"status": "Active"})
#     target_count = round(total_employees * 0.5)
...
#     # find every distinct year-quarter that has at least one Training Event
#     try:
#         rows = frappe.db.sql(
#             """
#             SELECT YEAR(start_time) AS y, QUARTER(start_time) AS q
#             FROM `tabTraining Event`
#             WHERE docstatus < 2 AND start_time IS NOT NULL
#             GROUP BY YEAR(start_time), QUARTER(start_time)
#             ORDER BY y DESC, q DESC
#             LIMIT 4
#             """
#         )
#     except Exception:
#         frappe.log_error(frappe.get_traceback(), "IMS training_stats periods")
#         rows = []

#     if not rows:
#         return {"year": None, "quarters": [], "total_employees": total_employees}

#     # oldest first for left-to-right chart order
#     periods = list(reversed(rows))  # [(y, q), ...]

#     out = []
#     for y, q in periods:
#         q_start_month = (q - 1) * 3 + 1
#         q_start = getdate("%s-%02d-01" % (y, q_start_month))
#         q_end = add_days(getdate(add_months(q_start, 3)), -1)
...
#             )[0][0] or 0
#         except Exception:
#             frappe.log_error(frappe.get_traceback(), "IMS training_stats %s-Q%s" % (y, q))
...
#     return {"year": None, "quarters": out, "total_employees": total_employees}
